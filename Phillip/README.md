...
| `all_source_code.ipynb` | Every module above, verbatim, regrouped into a readable **section-by-section** notebook (small blocks — config, task, dynamics, each paradigm, each analysis, plotting, orchestration — each with a markdown explanation on top, plus a few short runnable demos). For anyone on the team who only works in Jupyter: `Restart & Run All` defines the whole pipeline inline (the ~15–25 min full run is left commented out), then you can build/train/analyze/plot in cells below without touching a `.py` file. Generated from the `.py` files — if you edit code, prefer editing the `.py` file and regenerating this notebook so the two don't drift apart. |
